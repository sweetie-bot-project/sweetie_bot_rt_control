...
template <class ActionSpec> class OrocosSimpleActionServer 
{
	...
	public:
		...
		/**
		 * Create OrocosSimpleActionServer in given Service.
		 *
...
template <class ActionSpec> bool OrocosSimpleActionServer<ActionSpec>::start(bool publish_feedback) 
{
	if (ready()) {
		action_server.start();
...
template <class ActionSpec> bool OrocosSimpleActionServer<ActionSpec>::acceptPending(const Result& result, const std::string& msg)
{
	if (isPending()) {
