...
void ExecuteJointTrajectory::newGoalHook(const Goal& pending_goal) 
{
	...
	try {
		...
		case ModifiedAkima:
			goal_pending = std::make_shared<JointTrajectoryCache>(pending_goal, robot_model, ModifiedAkimaInterpolation(stop_threshold)); 
			break;
		case ModifiedCubic:
		...
		default:
			...
			goal_result.error_string = "Invalid interpolation algorithm type was selected";
