...
bool KinematicsFwd::configureHook()
{
	...
	for(auto &name: chain_names) {
		...
		// get kinematic chain
		data.chain = make_shared<KDL::Chain>( robot_model->getKDLChain(name, virtual_links) );  // if virtual_links is true receive chain with virtual links
		...
		// solvers
		data.fk_vel_solver = make_shared<ChainFkSolverVel_recursive>(*data.chain);
